...
time_series_collector.main(fx_tickers, "../datasetsIn/fx-daily")
print(f"Time series are collected in {time.time() - start_time} seconds.")

start_time = time.time()
print("Merging time series...")
time_series_merger.main(f"../datasetsIn/fx", "../datasetsOut/fx", "daily", len(fx_tickers))
print(f"Time series are merged in {time.time() - start_time} seconds.")

start_time = time.time()
print("Marking weeks...")
merged_file_name = next(os.walk("../datasetsOut/fx"))[2][0]
week_marker.main(f"../datasetsOut/fx/{merged_file_name}")
...
time_series_weekdays_filler.main(f"../datasetsOut/fx/{marked_file_name}")
