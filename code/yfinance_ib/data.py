...
class YfData(metaclass=SingletonMeta):
    ...
    def _get_cookie_and_crumb(self, proxy=None, timeout=30):
        """
        CHANGE: Simplified cookie/crumb retrieval to bypass complex authentication
        that was failing on some systems. Now returns None for cookie and crumb
        to let the direct request work.
        
        Original method attempted complex cookie/crumb strategies that were failing
        with "Expecting value: line 1 column 1 (char 0)" errors.
        """
        utils.get_yf_logger().debug("Using simplified cookie/crumb method (bypassing complex auth)")
        # Return None for both cookie and crumb to use simple requests
    ...
    def get(self, url, user_agent_headers=None, params=None, proxy=None, timeout=30):
        """
        CHANGE: Modified to work without requiring cookie/crumb authentication.
        Uses direct requests with proper headers that we know work from testing.
        """
        if len(url) > 200:
        ...
        # CHANGE: Skip crumb parameter entirely
        # Original code added crumb to params, now we don't
        
        request_args = {
            'url': url,
            'params': params,  # No crumb parameter
            'proxies': proxy,
            'timeout': timeout,
        ...
        try:
            ...
            utils.get_yf_logger().error(f"Request failed: {e}")
            # Return an empty response object that will trigger fallback
            class EmptyResponse:
                status_code = 400
                def json(self):
                    return {}
