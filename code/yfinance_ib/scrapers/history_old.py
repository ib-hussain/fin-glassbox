...
class PriceHistory:
    ...
    def history(self, period="1mo", interval="1d",
        ...
        logger.debug(f'{self.ticker}: yfinance_ib received OHLC data: {quotes.index[0]} -> {quotes.index[-1]}')
        ...
        logger.debug(f'{self.ticker}: yfinance_ib returning OHLC: {df.index[0]} -> {df.index[-1]}')
    ...
    def _fix_prices_sudden_change(self, df, interval, tz_exchange, change, correct_volume=False):
        ...
        if change in [100.0, 0.01] and f_splits.any():
            ...
            if f_pos.any():
                ...
                if gap_td < threshold:
                    logger.info('price-repair-split: 100x changes are too soon after stock split events, aborting')
                    return df
