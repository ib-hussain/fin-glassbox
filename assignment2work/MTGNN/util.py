...
class DataLoaderS(object):
    ...
    def __init__(
        ...
        self.rawdat = self._read_raw_data(file_name, week, num_weeks)
        if run_for_prediction:
            for i in range(self.horizon):self.rawdat = np.concatenate((self.rawdat, self.rawdat[-1:]))
        ...
        if run_for_prediction:self.all = self._batchify(range(self.window + self.horizon - 1, self.num_rows))
        self.scale = torch.from_numpy(self.scale).float()
        tmp = self.test[1] * self.scale.expand(self.test[1].size(0), self.num_cols)
        self.scale = self.scale.to(device)
        self.scale = Variable(self.scale)
        self.rse = normal_std(tmp)
        self.rae = torch.mean(torch.abs(tmp - torch.mean(tmp)))
        self.device = device
    def _read_raw_data(self, path, week, num_weeks, fill_zero_for_the_first_horizon_samples=False):
        fin = open(path)
    ...
    def _normalized(self, normalize, train_end_index):
        if normalize == 0:self.dat = self.rawdat
        # normalized by the maximum value of entire matrix.
        if normalize == 1:self.dat = self.rawdat / np.max(self.rawdat[:train_end_index])
        # normalized by the maximum value of each row(sensor).
        if normalize == 2:
            for i in range(self.num_cols):
                self.scale[i] = np.max(np.abs(self.rawdat[:train_end_index, i]))
                self.dat[:, i] = self.rawdat[:, i] / self.scale[i]
    def _split(self, train_end_index, valid_end_index):
        train_set = range(self.window + self.horizon - 1, train_end_index)
        valid_set = range(train_end_index, valid_end_index)
        test_set = range(valid_end_index, self.num_rows)
        self.train = self._batchify(train_set)
        self.valid = self._batchify(valid_set)
        self.test = self._batchify(test_set)
    def _batchify(self, idx_set):
        n = len(idx_set)
        X = torch.zeros((n, self.window, self.num_cols))
        Y = torch.zeros((n, self.num_cols))
        for i in range(n):
    ...
    def get_inputs(self, inputs, batch_size):
        length = len(inputs)
...
def calculate_scaled_laplacian(adj_mx, lambda_max=2, undirected=True):
    if undirected:  adj_mx = np.maximum.reduce([adj_mx, adj_mx.T])
...
def load_pickle(pickle_file):
    try:
        with open(pickle_file, "rb") as f:pickle_data = pickle.load(f)
    except UnicodeDecodeError as e: 
        with open(pickle_file, "rb") as f:pickle_data = pickle.load(f, encoding="latin1")
...
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    # Data format
    for category in ["train", "val", "test"]:  data["x_" + category][..., 0] = scaler.transform(data["x_" + category][..., 0])
...
def load_node_feature(path):
    ...
    return z
def normal_std(x):return x.std() * np.sqrt((len(x) - 1.0) / (len(x)))
# noinspection PyPep8Naming
def format_time_as_YYYYMMddHHmm(time):return (time.isoformat(sep="/", timespec="minutes").replace("-", "").replace(":", "").replace("/", ""))
