...
model_dir = f"{str(os.getenv("model_StemGNN_path","assignment2work/StemGNN/model" ))}"

def save_model(model, model_dir=model_dir, epoch=None):
    if model_dir is None:return
    if not os.path.exists(model_dir):os.makedirs(model_dir)
    epoch = str(epoch) if epoch else ""
    file_name = os.path.join(model_dir, epoch + "_stemgnn.pt")
    with open(file_name, "wb") as f:torch.save(model, f)
def load_model(model_dir=model_dir, epoch=None):
    if not model_dir:return
    epoch = str(epoch) if epoch else ""
    file_name = os.path.join(model_dir, epoch + "_stemgnn.pt")
    if not os.path.exists(model_dir):os.makedirs(model_dir)
    if not os.path.exists(file_name):return
    with open(file_name, "rb") as f:model = torch.load(f)
    return model
    ...
    with torch.no_grad():
        for i, (inputs, target) in enumerate(dataloader):
            ...
            while step < horizon:
                ...
                forecast_steps[:, step:min(horizon - step, len_model_output) +
                               step, :] = (forecast_result[:, :min(horizon -
                                                                   step, len_model_output), :].detach().cpu().numpy())
...
def validate(
    ...
    window_size,
    horizon,
    result_file=f"{str(os.getenv("result_file_StemGNN_path", "assignment2work/StemGNN/output" ))}",
):
    start = datetime.now()
    ...
    else:forecast, target = forecast_norm, target_norm
    ...
    print(f"RAW : MAPE {score[0]:7.9%}; MAE {score[1]:7.9f}; RMSE {score[2]:7.9f}; A20 {score[3]:7.9f}.")
    if result_file:
...
def train(train_data, valid_data, args, result_file=f"{str(os.getenv("result_file_StemGNN_path", "assignment2work/StemGNN/output" ))}"):
    node_count = train_data.shape[1]
    model = Model(node_count, 2, args.window_size, args.multi_layer, horizon=args.horizon)
    model.to(args.device)
    if len(train_data) == 0:raise Exception("Cannot organize enough training data")
    if len(valid_data) == 0:raise Exception("Cannot organize enough validation data")
    ...
    else:normalize_statistic = None
    if normalize_statistic is not None:
        with open(os.path.join(result_file, "norm_stat.json"), "w") as f:
            json.dump(normalize_statistic, f)

    if args.optimizer == "RMSProp":my_optim = torch.optim.RMSprop(params=model.parameters(), lr=args.lr, eps=1e-08)
    else:my_optim = torch.optim.Adam(params=model.parameters(), lr=args.lr, betas=(0.9, 0.999))
    ...
    performance_metrics = {}
    for epoch in tqdm(range(args.epoch), desc="handler.py > epoch"):
        ...
        print("| end of epoch {:3d} | time: {:5.2f}s | train_total_loss {:5.5f}".format(epoch, (time.time() - epoch_start_time), loss_total / cnt))
        if (epoch + 1) % args.exponential_decay_step == 0:my_lr_scheduler.step()
        ...
        # early stop
        if (args.early_stop and validate_score_non_decrease_count >= args.early_stop_step):break
    return performance_metrics, normalize_statistic
def test(test_data, args, result_train_file=f"{str(os.getenv("result_file_StemGNN_path", "assignment2work/StemGNN/output" ))}/train", result_test_file=f"{str(os.getenv("result_file_StemGNN_path", "assignment2work/StemGNN/output" ))}/test"):
    with open(os.path.join(result_train_file, "norm_stat.json"), "r") as f:normalize_statistic = json.load(f)
    ...
    mae, mape, rmse, a20 = (
        ...
        performance_metrics["a20"],
    )
    print(
        "Performance on test set: MAPE: {:5.5f} | MAE: {:5.5f} | RMSE: {:5.5f} | A20: {:5.5f}".format(mape, mae, rmse, a20),
        flush=True,
    )
    return performance_metrics["rmse"]
